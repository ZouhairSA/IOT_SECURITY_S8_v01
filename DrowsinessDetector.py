...
class DrowsinessDetector(QMainWindow):
    ...
    def update_ui(self):
        """Update UI elements and save stats to MongoDB only if drowsiness is detected"""
        ...
        # Save statistics to MongoDB only if an alert (drowsiness case) is triggered
        if self.alert_text:  # Condition pour vérifier si un cas de somnolence est détecté
            stats_data = {
            ...
            self.db.insert_data("drowsiness_stats", stats_data)
    ...
    def closeEvent(self, event):
        ...
        if self.arduino and self.arduino.is_open:
            self.arduino.close()
        self.db.close()  # Fermer la connexion à MongoDB
        event.accept()
